    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
         <pre><strong>E-mail : </strong>{{$er['email']}}    </pre>
          <pre><strong>Company Name : </strong>{{$er['company_name']}}    </pre>
        <pre><strong>Phone Number : </strong>{{$er['phone']}}    </pre>
                    ...
                    <td>
                      <?php $test = App\Reply::where('contact_id',$contact->id)->orderBy('id','desc')->get()->first();
                         ...
                         @if($test=='')
                          ...
                          <div class="alert alert-danger" style="padding: 2px; width: 80px;" >返信されていません</div>


                        @elseif($test['status']=='active')

                       
                        <div class="alert alert-success" style="padding: 2px; width: 55px;" > 返信済み</div>
...
<div class="modal fade" id="view{{$er->id}}">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title" id="view"><strong> :</strong>  {{$er['name']}} </h3>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
         <pre><strong>電子メールアドレス : </strong>{{$er['email']}}    </pre>
         <pre><strong> 電話番号: </strong>{{$er['phone']}}    </pre>
       
       <p><strong>会社名: <br></strong>{!!$er['company_name']!!} </p>
        <p><strong>メッセージ : <br></strong>{!!$er['message']!!} </p>
      </div>
      <div class="modal-footer">
         <a href="{{url('/replycontact/'.$er->id)}}"><button type="submit" class="btn btn-primary pull-left">応答</button></a>
...
<div class="modal modal-danger fade" id="modal-danger{{$er['id']}}">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <p>削除してもよろしいですか ?&hellip;</p>
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">番号</button>
              <a href="{{url('/deleteinbox/'.$er['id'])}}"> <button type="button" class="btn btn-outline">はい</button></a>
              </div>
            </div>
            <!-- /.modal-content -->
          </div>
